...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    ____TIME_STAMP(3); // ____TIME_STAMP(3) must always come before pim->write_sreg()
    pim->write_sreg(0,0);
    ____TIME_STAMP(5); // ____TIME_STAMP(5) must always come before pim->start_computation()
    ...
    while (op_count < TOTAL_NUM_OPS){
        ...
        if (rand_temp <= READ_ONLY_PERCENTAGE){
            ...
            pim_res = pim->read_sreg(2);                    // _______________________________
